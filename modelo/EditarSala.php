...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Actualizar espacio
        if ($imagen) {
            $stmt = $conn->prepare("UPDATE espacio SET nom_sala = ?, capacidad = ?, imagen = ?, centro = 'secundaria' WHERE id_espacio = ?");
            $stmt->bind_param("sisi", $nom_sala, $capacidad, $imagen, $id_espacio);
        } else {
            $stmt = $conn->prepare("UPDATE espacio SET nom_sala = ?, capacidad = ?, centro = 'secundaria' WHERE id_espacio = ?");
